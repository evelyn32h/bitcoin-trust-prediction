...
from src.preprocessing import filter_neutral_edges, map_to_unweighted_graph, ensure_connectivity, normalize_edge_weights, reindex_nodes_sequentially
from src.feature_extraction import feature_matrix_from_graph
from src.models import train_edge_sign_classifier, print_model_info
...
def train_and_test_model(num_edges_to_suppress=1, threshold=0.5):
    ...
    G = reindex_nodes_sequentially(G)
    ...
    X_training, y_training, _ = remove_edges(X, y, edges, removed_edges)
    
    # Train the model
    model = train_edge_sign_classifier(X_training, y_training)
    print("Model training complete.")
    
    # Predict and evaluate the suppressed edges
    cml_evaluation(model, X, y, edges, removed_edges, threshold=threshold)
